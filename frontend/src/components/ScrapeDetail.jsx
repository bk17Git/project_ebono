...
const ScrapeDetail = ({ user }) => {
  ...
  useEffect(() => {
    document.title = "Comute Details | Comunce";
 }, []); 
 ...
 const goTohome = () => {
  ...
  if (!scrapeDetail) {
    return <Spinner message="Showing  Comute" />;
  }
  ...
  return (
    <div className="md:mt-5">
        ...
        >
          ...
          <div className="w-full p-5 flex-1 xl:min-w-620">
            <div className="flex items-center justify-between">
              ...
              <p>Category: {scrapeDetail.category}</p>
            </div>
            <div>
              <p className="mt-3">{scrapeDetail.caption}</p>
            </div>
            <Link
              to={`/user-profile/${scrapeDetail?.postedBy._id}`}
              className="flex gap-2 mt-5 items-center bg-white rounded-lg "
            >
              <img
                src={scrapeDetail?.postedBy.image}
      ...
      {scrapes ? (
        ...
        <Spinner message="Loading more comutes" />
      )}
    </div>
  );
};
